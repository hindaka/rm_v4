...
$list_data = $db->query("SELECT rp.tanggaldaftar,pul.tgl_pulang,rp.nomedrek,rp.nama,rp.kelamin,rp.tanggallahir,rp.jpasien,rp.domisili,rp.ktujuan,pul.kasus,pul.diagnosa_akhir,rf.diagnosa_utama,rf.kode_icd,rf.diagnosa_tambahan,pul.asal,pul.cara_keluar FROM registerpasien_pulang pul INNER JOIN registerpasien rp ON(pul.id_register=rp.id_pasien) INNER JOIN registerpasien_final rf ON(rf.id_pulang=pul.id_pulang) WHERE pul.jenis_rawat='rajal' AND pul.final='y' AND tgl_pulang between '".$awal."' and '".$akhir_up."'");
$data = $list_data->fetchAll(PDO::FETCH_ASSOC);
//EXCEL
header("Content-type: application/vnd.ms-excel");
header("Content-Disposition: attachment; filename=rekap_coding_rajal-".$awal."-".$akhir.".xls");
?>
Rekap Coding Rawat Jalan <?php echo $awal; ?> sampai dengan <?php echo $akhir; ?>
