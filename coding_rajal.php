...
$gabung = "$tahun-$bulan";
if($jpasien=='Umum'){
		$list_data = $db->query("SELECT * FROM registerpasien_pulang pul INNER JOIN registerpasien rp ON(pul.id_register=rp.id_pasien) WHERE pul.jenis_rawat='rajal' AND pul.final='n' AND pul.tgl_pulang LIKE '%".$gabung."%' AND rp.jpasien='Umum'");
}else{
		$list_data = $db->query("SELECT * FROM registerpasien_pulang pul INNER JOIN registerpasien rp ON(pul.id_register=rp.id_pasien) WHERE pul.jenis_rawat='rajal' AND pul.final='n' AND pul.tgl_pulang LIKE '%".$gabung."%' AND rp.jpasien<>'Umum'");
}
        ...
        <section class="content">
          ...
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">Data Pasien Pulang Rawat Jalan</h3>
            </div>
            <div class="box-body">
											...
											<?php
												foreach ($data as $row) {
													echo "<tr>
														...
														<td>
															<a class='btn btn-primary btn-sm' href='coding_rajal_acc.php?pul=".$row['id_pulang']."&bln=".$bulan."&thn=".$tahun."&jpasien=".$jpasien."' data-toggle='tooltip' data-placement='left' title='Klik pilih untuk melanjutkan'><i class='fa fa-send'></i> Pilih</a>
														</td>
													</tr>";
												}
											?>
