...
$die = $db->query("SELECT rp.tanggaldaftar,rp.jamdatang,rp.nama,rp.nomedrek,rp.namapp,rp.tanggallahir,rp.umur,rp.alamat,rp.kelurahan,rp.kecamatan,rp.kelamin,rp.rujukan,rpp.* FROM registerpasien_pulang rpp INNER JOIN registerpasien rp ON rpp.id_register=rp.id_pasien WHERE rpp.cara_keluar='Meninggal' AND rpp.tgl_pulang LIKE '%$gabung%'");
$die_fix = $die->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html>
  <head>
        ...
        <section class="content">
          ...
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">Rekap Data Pasien Meninggal</h3>
            ...
            <div class="box-body">
							<div class="table-responsive">
								<table id="example1" class="table table-bordered table-hover table-striped">
									...
									<tbody>
										<?php
											foreach ($die_fix as $row) {
												if(strlen($row['tanggaldaftar'])==10){
													$masuk = explode("/",$row['tanggaldaftar']);
													$tgl_in = $masuk[2]."-".$masuk[1]."-".$masuk[0]." ".$row['jamdatang'];
												}else{
													$masuk = $row['tanggalmasuk'];
													$tgl_in = $masuk;
												}
												if($row['tgl_pulang']=="0000-00-00 00:00:00"){
													$check = $db->query("SELECT COUNT(*) as total FROM log_trans_pasien WHERE id_register='".$row['id_register']."' AND keluar LIKE '%".$row['carakeluar']."%'");
													$ch = $check->fetch(PDO::FETCH_ASSOC);
													if($ch['total']==0){
														$tgl_out = $row['tgl_pulang'];
														$lama_rawat = "unknown";
													}else{
														$get_data = $db->query("SELECT * FROM log_trans_pasien WHERE id_register='".$row['id_register']."' AND keluar LIKE '%".$row['carakeluar']."%'");
